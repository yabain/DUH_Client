...
**Note**: use the .env.sample file to create .env.development and .env.production, these files are used to set some env variables for both the development and production builds:

- REACT_APP_API_ENDPOINT: The API root endpoint (hosted on our custom server or lambda)
- REACT_APP_MESSAGE_ENDPOINT: Our message root endpoint hosted on AWS API Gateway (Lambda functions)
- REACT_APP_SOCKET_ENDPOINT: The socket root endpoint for real-time chat/messaging hosted on AWS API Gateway (Lambda functions)
- REACT_APP_CAPTCHA_CLIENT_KEY: The Google Re-Captcha client key.
...
The project uses [read icons](https://react-icons.github.io/) to show some special icons to better the UI.
Here's the list of [Bootst icons](https://react-icons.github.io/icons?name=bs) that we'll preferably include in the App.
